...
async function seedDefaultData(client) {
  ...
  for (const p of saProvinces) {
    await client.query(
      'INSERT INTO provinces (country_id, name) VALUES ((SELECT id FROM countries WHERE code = $1), $2) ON CONFLICT (country_id, name) DO NOTHING',
      ['ZA', p]
    );
  }
  ...
  for (const cur of curricula) {
    await client.query(
      'INSERT INTO curricula (country_id, name) VALUES ((SELECT id FROM countries WHERE code = $1), $2) ON CONFLICT (country_id, name) DO NOTHING',
  ...
  for (const g of gradeData) {
    ...
    if (levelId.rows.length) {
      for (const name of g.names) {
        await client.query(
          'INSERT INTO grades (education_level_id, name, display_name) VALUES ($1, $2, $2) ON CONFLICT (education_level_id, name) DO NOTHING',
  ...
  // Subjects for CAPS (curriculum_id = 1)
  const capsId = await client.query('SELECT id FROM curricula WHERE name = $1 AND country_id = (SELECT id FROM countries WHERE code = $2)', ['CAPS', 'ZA']);
    ...
    for (const gname of gradeNames) {
      ...
      if (gradeRow.rows.length) {
        ...
        for (const sub of subjects) {
          await client.query(
            'INSERT INTO subjects (curriculum_id, grade_id, name) VALUES ($1, $2, $3) ON CONFLICT (curriculum_id, grade_id, name) DO NOTHING',
            [cid, gid, sub]
          );
        }
      }
  ...
  // Add more curricula (IEB, etc.) similarly if needed.
}
